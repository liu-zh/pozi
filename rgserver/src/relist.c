...
while(q = p->next)
p = p->next;
struct regist_node* n = 
       (struct regist_node*)malloc(sizeof(struct regist_node));
...
**/
/*****************/
void getRegistNode(struct regist_node* head,unsigned int index,struct meta_msg* v){
if(!head){
printf("Linklist be not initialized\n");
exit(-1);
}
