...
int server_listen(const int port, const int backlog, int nonblock) {
	...
	if (bind(servfd, (struct sockaddr*) &servaddr, sizeof(struct sockaddr_in))
			== -1) {
...
int client_connect(const char* ipaddr, const int port) {
	...
	if (connect(cliefd, (const struct sockaddr*) &clieaddr,
			sizeof(struct sockaddr_in)) == -1) {
		printf("connect to server failed! %s\n", strerror(errno));
		exit(-1);
	}
	printf("connection is established! to %s:%d\n", ipaddr, port);
	return cliefd;
}
/**********************************/
