...
struct dir_list_node* n =
       (struct dir_list_node*)malloc(sizeof(struct dir_list_node));
...
/*****************/
void getDirNode(struct dir_list_node* head,unsigned int index,struct dir_node_value* v){
if(!head){
printf("Linklist be not initialized\n");
exit(-1);
}
